...
export function generateValidator() {
    ...
    const entityFileName = rl.question(
        "Enter Entity File Name (example : User or Folder/EntityB if inside a nested folder) :"
    );

    const existingFileAnswer = rl.question(
        "Is this an initial validation? or the file already present? (Y/y for yes, other answer for no) :"
    );
    ...
    while (stillInput) {
        ...
        const answer = rl.question(
            "Enter field name (example : fullName) , leave empty and press enter when you are done :"
        );
        if (answer === "" || answer === "\n") {
            stillInput = false;
        }
