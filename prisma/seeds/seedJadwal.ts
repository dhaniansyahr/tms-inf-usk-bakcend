  ...
  matakuliah.forEach((mk) => {
    const eligible = allMahasiswa.filter(
      (mahasiswa) =>
        ...
        (studentWorkload.get(mahasiswa.id) || 0) + mk.sks <= MAX_SKS_PER_STUDENT
    );
...
export async function seedJadwal(prisma: PrismaClient) {
  ...
  console.log(
    `\n🎯 STARTING FAIR DISTRIBUTION FOR ${matakuliah.length} THEORY COURSES`
  );
  console.log(
    `📚 Available: ${mahasiswa.length} mahasiswa, ${dosen.length} dosen, ${availableRuangan.length} ruangan`
  );
  ...
  if (successfulSchedules.length > 0) {
    ...
    console.log(
      `   Average students per course: ${(
        successfulSchedules.reduce((sum, s) => sum + s.enrolledStudents, 0) /
