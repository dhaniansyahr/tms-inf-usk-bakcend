...
export async function update(data: AclDTO): Promise<ServiceResponse<CreateResponse>> {
        try {
                // Validate that the user level exists
                const userLevel = await prisma.userLevels.findUnique({
                        where: { id: data.userLevelId },
                });
                ...
                const existingAcls = await prisma.acl.findMany({
                        where: { userLevelId: data.userLevelId },
                        select: { namaFeature: true, namaAction: true },
                });

                const existingPermissionKeys = new Set(existingAcls.map((acl) => `${acl.namaFeature}:${acl.namaAction}`));
                ...
                const permissionsToRemove = existingAcls.filter((acl) => !newPermissionKeys.has(`${acl.namaFeature}:${acl.namaAction}`));
...
export async function addPermissions(userLevelId: string, permissions: { subject: string; action: string[] }[]): Promise<ServiceResponse<{}>> {
        try {
                // Validate that the user level exists
                const userLevel = await prisma.userLevels.findUnique({
...
export async function removePermissions(userLevelId: string, permissions: { subject: string; action: string[] }[]): Promise<ServiceResponse<{}>> {
        try {
                // Validate that the user level exists
                const userLevel = await prisma.userLevels.findUnique({
                        where: { id: userLevelId },
