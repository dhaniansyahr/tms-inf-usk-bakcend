...
): Promise<ServiceResponse<UpdateMeetingResponse>> {
  try {
...
export async function getMeetingById(
  id: string
): Promise<ServiceResponse<MeetingWithJadwalDTO | {}>> {
  try {
 ...
 */
export async function checkMeetingUpdateEligibility(
  id: string
): Promise<
  ServiceResponse<
    { canUpdate: boolean; reason?: string; daysUntilMeeting: number } | {}
  >
> {
  try {
    const meeting = await prisma.meeting.findUnique({
 ...
 */
export async function getAllMeetingsByJadwalId(
  jadwalId: string,
  filters: FilteringQueryV2
): Promise<ServiceResponse<GetAllMeetingsResponse>> {
  try {
 ...
 */
export async function getMeetingsByJadwalId(
  jadwalId: string
...
export async function getListMahasiswaByJadwalId(
  jadwalId: string
...
export async function getListParticipantsByJadwalId(
  jadwalId: string
): Promise<ServiceResponse<{}>> {
  try {
    ...
    const dosenWithMeetings = dosenList.map((dosen) => {
      const dosenMeetings = meetings.map((meeting) => {
        ...
        const absensi = absensiRecords.find(
          (record) =>
            record.dosenId === dosen.id && record.meetingId === meeting.id
        );
