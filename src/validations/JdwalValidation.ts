...
export async function validateJadwal(c: Context, next: Next) {
        ...
        if (!data.dosenIds) invalidFields.push(generateErrorStructure("dosenIds", "Dosen tidak boleh kosong"));
        ...
        // check if dosenIds is array
        if (!Array.isArray(data.dosenIds)) invalidFields.push(generateErrorStructure("dosenIds", "Dosen harus berupa array"));
