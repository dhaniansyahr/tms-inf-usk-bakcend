...
export async function validatePendaftaranAsistenLab(c: Context, next: Next) {
  ...
  if (!data.mahasiswaId)
    invalidFields.push(
      generateErrorStructure("mahasiswaId", "mahasiswaId tidak boleh kosong")
    );
  ...
  if (!jadwalExist)
    return invalidFields.push(
      generateErrorStructure("jadwalId", "jadwalId tidak ditemukan")
    );
  ...
  if (existingRegistration)
    return invalidFields.push(
      generateErrorStructure(
        "mahasiswaId",
        "mahasiswaId sudah terdaftar sebagai asisten lab untuk jadwal ini"
      )
    );

  if (!data.nilaiTeori)
    invalidFields.push(
      generateErrorStructure("nilaiTeori", "nilaiTeori tidak boleh kosong")
    );
  ...
  if (!data.nilaiAkhir)
    invalidFields.push(
      generateErrorStructure("nilaiAkhir", "nilaiAkhir tidak boleh kosong")
    );
...
export async function validatePenerimaanAsistenLab(c: Context, next: Next) {
  ...
  if (data.status === ASISTEN_LAB_STATUS.DITOLAK && !data.keterangan)
    invalidFields.push(
      generateErrorStructure("keterangan", "keterangan tidak boleh kosong")
    );
