...
import { DosenDTO, MahasiswaDTO } from "$entities/MasterData";
...
export async function validateMahasiswa(c: Context, next: Next) {
    ...
    if (!isNPM) {
        invalidFields.push(
            generateErrorStructure(
                "npm",
                "npm format is invalid, expected 13 digits"
            )
        );
    }

    if (!data.npm)
        invalidFields.push(generateErrorStructure("npm", "npm is required"));
    if (!data.nama)
        invalidFields.push(generateErrorStructure("nama", "nama is required"));
    if (!data.password)
        invalidFields.push(
    ...
    if (!data.semester)
        invalidFields.push(
            generateErrorStructure("semester", "semester is required")
        );
    if (!data.tahunMasuk)
        invalidFields.push(
            generateErrorStructure("tahunMasuk", "tahunMasuk is required")
        );
...
export async function validateDosen(c: Context, next: Next) {
    ...
    if (!isNIP) {
        invalidFields.push(
            generateErrorStructure(
                "nip",
                "nip format is invalid, expected 16 digits"
            )
        );
    }

    if (!data.nip)
        invalidFields.push(generateErrorStructure("nip", "nip is required"));
    if (!data.nama)
        invalidFields.push(generateErrorStructure("nama", "nama is required"));
    if (!data.password)
        invalidFields.push(
